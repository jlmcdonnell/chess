...
import dev.mcd.chess.api.domain.opponent
import dev.mcd.chess.api.domain.sideForUser
...
class JoinOnlineGameImpl @Inject constructor(
    ...
    override suspend fun invoke(id: GameId) = channelFlow {
        val userId = prefs.userId() ?: throw Exception("No user ID")
        val authToken = prefs.token() ?: throw Exception("No auth token")
